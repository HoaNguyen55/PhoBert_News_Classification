...
# train_dataset = tf.data.Dataset.from_tensor_slices((ids_train, y_train))
# test_dataset = tf.data.Dataset.from_tensor_slices((ids_test, y_test))
# BATCH_SIZE = 8
...
# model = Sequential()
# model.add(Conv1D(128, kernel_size=5, input_shape=(10,768)))
...
print(model.summary())
# # fit the model
# X = np.asarray(train_dataset).astype(np.float32)
# Y = np.asarray(test_dataset).astype(np.float32)
model.fit_generator(train_generator, epochs=10, verbose=1, validation_data=valid_generator)
# # evaluate the model
loss, accuracy = model.evaluate_generator(test_dataset, verbose=1)
