    ...
    def train_classifier(self):
        ...
        for epoch_i in range(0, self.epochs):
            ...
            if best_valid_loss > eval_loss:
                ...
                best_valid_loss_path = "{}/model_best_valoss.pth".format(self.save_dir)
                self.save_checkpoint(best_valid_loss_path)
            if best_eval_accuracy > eval_accuracy:
                best_eval_accuracy = eval_accuracy
                best_eval_accuracy_path = "{}/model_best_valacc.pth".format(self.save_dir)
                self.save_checkpoint(best_eval_accuracy_path)
            
                epoch_i_path = "{}/model_epoch{}.pth".format(self.save_dir, epoch_i)
                self.save_checkpoint(epoch_i_path)
                os.remove("{}/model_epoch{}.pth".format(self.save_dir, epoch_i-1))
...
def main():
    ...
    history = bert_classifier_trainer.load_checkpoint(bert_classifier_trainer.save_dir)
    ...
    plt.show()
    # Plot model and summary information, model loss
